...
function ClientsPage() {
  ...
  const { data = { items: [], totalItems: 0, totalPages: 1 }, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`http://localhost:5157/api/Clients?search=${encodeURIComponent(search)}&page=${page}&pageSize=${pageSize}`);
  ...
  const { data: stats = { totalClients: 0, activeGuests: 0, totalRevenue: 0 } } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch("http://localhost:5157/api/Clients/stats");
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => {
      const res = await fetch(`http://localhost:5157/api/Clients/${id}`, { method: "DELETE" });
...
function EditClientSheet({ client }: { client: any }) {
  ...
  const updateMutation = useMutation({
    mutationFn: async (updated: any) => {
      const res = await fetch(`http://localhost:5157/api/Clients/${client.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
